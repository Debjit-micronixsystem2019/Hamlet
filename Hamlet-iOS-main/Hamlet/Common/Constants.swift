//
//  Constants.swift
//  Hamlet
//
//  Created by Amit on 10/09/21.
//  Copyright © 2021 Admin. All rights reserved.
//

import Foundation

struct Constants {
    static let done = "Done"
    static let cancel = "Cancel"
    static let ok = "OK"
    static let yes = "YES"
    static let no = "NO"
    static let fitnessApp = "Fitness App"
    static let alertTitle = "Alert"
    static let noInternet = "No Internet"
    static let noData = "No data available"
    static let invalidRequest = "Invalid request parameters"
    static let invalidURL = "Invalid URL"
    static let updationError = "Failed to updatet"
    static let emailExists = "Email id already exists"

    static let fillDetail = "Please fill all the required detail."
    static let invalidCredential = "Invalid credential."
    static let invalidEmail = "Please enter valid email id."
    static let smallPassword = "Password is too small."
    static let enterPassword = "Please enter a long password."
    static let doesNotMatchPassword = "Password doesn't match."
    static let smallName = "Please enter a valid name."
    static let noProblemSelection = "Please select any problems."
    static let noExpertiseSelection = "Please select any expertise."
    static let noUserSelection = "Please select users for send this group invitation."

}
